...
const generateHTMLDocument = (
    ...
    const memberCategoryRoles = new Set(['member', 'president', 'vice_president']);
    ...
    // Others present: coordinator, observer, guest, elected_official who are present
    const othersPresent = meetingAttendees.filter(a => a.isPresent && !isMemberRole(a.role));

    // Absents: only the members in the meeting's attendance list who are marked as not present
    const absents = meetingAttendees.filter(a => !a.isPresent && isMemberRole(a.role) && !excludeFromAbsents.has(normalizeRole(a.role)));
